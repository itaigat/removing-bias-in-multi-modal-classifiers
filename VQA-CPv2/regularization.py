...
class Perturbation(object):
    @classmethod
    def _add_noise_to_tensor(cls, tens: torch.Tensor, over_dim: int = 1) -> torch.Tensor:
        """
...
class Regularization(object):
    ...
    @classmethod
    def get_batch_score(cls, grad: torch.Tensor, loss: torch.Tensor = None, estimation: str = 'ent',
                        n_samples: int = 10, grad_est: bool = True) -> torch.Tensor:
        if grad_est:
            return cls.get_batch_norm(grad=grad, loss=loss, estimation=estimation)
        else:
            return cls.get_batch_statistics(loss=loss, n_samples=n_samples, estimation=estimation)
